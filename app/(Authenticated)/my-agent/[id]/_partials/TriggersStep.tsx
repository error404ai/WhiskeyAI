          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {agentTriggers?.map((trigger) => (
              <Card key={trigger.id} className="h-[180px] transition-shadow duration-300 hover:shadow-lg">
                ...
                <CardContent>
                  <p className="text-muted-foreground mb-2 text-sm line-clamp-2">{trigger.description}</p>
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} defaultValue="basic" className="w-full">
                      ...
                      <TabsContent value="function" className="space-y-4">
                        <label className="flex flex-col gap-2">
                          ...
                          {methods.formState.errors.functionName && (
                            <p className="text-sm text-red-500">{methods.formState.errors.functionName.message}</p>
                          )}
                        </label>
                        <label className="flex flex-col gap-2">
                          <span className="text-sm font-medium">Where to get tweet information from?</span>
                          <select
                            {...methods.register("informationSource")}
                            className="border-input bg-background ring-offset-background placeholder:text-muted-foreground focus-visible:ring-ring flex h-10 w-full rounded-md border px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                            defaultValue=""
                          >
                            <option value="" disabled>
                          ...
                          {methods.formState.errors.informationSource && (
                            <p className="text-sm text-red-500">{methods.formState.errors.informationSource.message}</p>
                          )}
                        </label>
                      </TabsContent>
                    </Tabs>
