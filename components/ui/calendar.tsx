    ...
    <DayPicker
      ...
      components={{
        ...
        Nav: ({ className }) => {
          ...
          return (
            <nav className={cn("flex items-center", className)}>
              <Button variant="outline" parentClass="absolute left-0 h-7 w-7 bg-transparent p-0 opacity-80 hover:opacity-100" className="absolute right-0 h-7 w-7" type="button" tabIndex={isPreviousDisabled ? undefined : -1} disabled={isPreviousDisabled} aria-label={navView === "years" ? `Go to the previous ${displayYears.to - displayYears.from + 1} years` : labelPrevious(previousMonth)} onClick={handlePreviousClick}>
                <ChevronLeft className="h-4 w-4" />
              </Button>

              <Button variant="outline" parentClass="absolute right-0 h-7 w-7 bg-transparent p-0 opacity-80 hover:opacity-100" className="absolute right-0 h-7 w-7" type="button" tabIndex={isNextDisabled ? undefined : -1} disabled={isNextDisabled} aria-label={navView === "years" ? `Go to the next ${displayYears.to - displayYears.from + 1} years` : labelNext(nextMonth)} onClick={handleNextClick}>
        ...
        CaptionLabel: ({ children, ...props }) => {
          ...
          return (
            <Button className="h-7 w-full truncate text-sm font-medium" variant="ghost" size="sm" onClick={() => setNavView((prev) => (prev === "days" ? "years" : "days"))}>
              ...
              <div className={cn("grid grid-cols-4 gap-y-2", className)} {...props}>
                {Array.from({ length: displayYears.to - displayYears.from + 1 }, (_, i) => {
                  ...
                  return (
                    <Button
                      key={i}
                      className={cn("text-foreground h-7 w-full text-sm font-normal", displayYears.from + i === new Date().getFullYear() && "bg-accent text-accent-foreground font-medium")}
                      variant="ghost"
                      onClick={() => {
                        setNavView("days");
